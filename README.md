...
نسخه فعال در پیام شروع و فرمان `/version` نمایش داده می‌شود. نسخه فعلی **3.1.0** است. پس از اولین اجرای موفق هر نسخه جدید، مدیر اصلی یک اعلان فارسی شامل شماره نسخه و فهرست تغییرات دریافت می‌کند. آخرین نسخه‌ای که اعلان شده در Redis نگهداری می‌شود تا با هر restart پیام تکراری ارسال نشود.
...
گزارش تغییرات هر پیج فقط برای مالک همان پیج ارسال می‌شود و نسخه‌ای از آن برای مدیر کپی نمی‌شود. خطاهای دسترسی اینستاگرام نیز دیگر به‌صورت پیام ناخواسته برای مدیر ارسال نمی‌شوند و فقط در بخش «لاگ کامل و عیب‌یابی» ثبت می‌گردند. اعلان نسخه جدید و فیش‌های پرداخت، به‌دلیل ماهیت مدیریتی، همچنان برای مدیر اصلی ارسال می‌شوند.
...
چکر درخواست‌های Web Profile API را از پراکسی داخلی WARP عبور می‌دهد. پیش از هر چرخه، ابتدا `warp=on` و سپس دسترسی واقعی به endpoint اینستاگرام بررسی می‌شود. HTTPX/HTTP2 مسیر اصلی است و curl بدون shell، رمز، Session یا grep به‌عنوان fallback اجرا می‌شود:
