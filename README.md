...
برای ارتقا از نسخه‌های قدیمی که updater آن‌ها روی تغییرات محلی متوقف می‌شود، فقط یک‌بار این فرمان واحد را اجرا کنید:

```bash
curl -fsSL https://raw.githubusercontent.com/farstar-team/farstar-warner/main/update.sh | sudo bash
```

این bootstrap ابتدا آخرین مدیر سرور را نصب می‌کند و بلافاصله backup و نصب آخرین نسخه را انجام می‌دهد. از آپدیت‌های بعدی فقط `farstar update` لازم است.
...
از نسخه ۵.۲.۱ همین یک فرمان پیش از تغییر سورس از تمام دیتابیس‌های در حال اجرا backup می‌گیرد، تغییرات محلی را در stash و یک شاخه پشتیبان نگه می‌دارد، سورس را دقیقاً با `origin/main` همگام می‌کند، image جدید می‌سازد و ربات‌های فعال را دوباره ایجاد می‌کند. بنابراین وجود تغییرات tracked روی سرور دیگر باعث توقف آپدیت نمی‌شود.
...
نسخه فعال در پیام شروع و فرمان `/version` نمایش داده می‌شود. نسخه فعلی **5.2.1** است. پس از اولین اجرای موفق هر نسخه جدید، مدیر اصلی یک اعلان فارسی شامل شماره نسخه و فهرست تغییرات دریافت می‌کند. آخرین نسخه‌ای که اعلان شده در Redis نگهداری می‌شود تا با هر restart پیام تکراری ارسال نشود.
...
| `PAGE_CHECK_DELAY_MAX_SECONDS` | خیر | `45` | بیشترین فاصله میان دو بررسی |
