...
نسخه **۵.۴.۰** کاملاً OSINT-only است و فقط داده‌هایی را تحلیل می‌کند که در Web Profile، Embed، جست‌وجوی مهمان یا HTML عمومی دیده می‌شوند. ربات هیچ رمز عبور، کوکی، `sessionid` یا توکن Meta دریافت نمی‌کند و برای پایش نیز به سرویس پولی متکی نیست.

### چه چیزی در نسخه ۵.۴.۰ اضافه شد؟

- در پاسخ محدود یا نامشخصِ مرحله ثبت، دکمه شیشه‌ای `🔄 تلاش مجدد برای بررسی پیج` نمایش داده می‌شود و زمان باقی‌مانده تا تلاش مجاز بعدی را به فارسی اعلام می‌کند.
- تلاش مجدد برای هر کاربر و username قفل مستقل و فاصله ۳۰ ثانیه‌ای دارد. هنگام اختلال سراسری فقط شاهد مثبت و معتبر فعال‌بودن پذیرفته می‌شود؛ پاسخ منفی یا مبهم در این مسیر وضعیت غیرفعال قطعی نمی‌سازد.
- نخستین ۴۰۴ دیگر اجرای مسیرها را متوقف نمی‌کند. هر پاسخ JSON معتبر ۲۰۰ اولویت دارد و نبودن پیج باید با مسیرهای مستقل WARP/مستقیم یا Curl/HTTP2 مستقیم به اجماع برسد.
- نبودن username در نتایج محدود و رتبه‌بندی‌شدهٔ جست‌وجوی مهمان فقط برای ممیزی Searchability استفاده می‌شود و هرگز به‌تنهایی پیج فعال را دی‌اکتیو نمی‌کند. پاسخ ۴۰۱/۴۰۳/۴۲۹ نیز حتی با بدنه JSON، شاهد تغییر وضعیت نیست.
- پنل مدیر دارای پیام همگانی پایدار با پیش‌نمایش، تأیید نهایی، شمارنده پیشرفت، توقف تأییدشده و تلاش دوباره برای تحویل‌های ناموفق است. وضعیت گیرندگان در PostgreSQL می‌ماند و پس از restart ادامه پیدا می‌کند؛ کمپین‌های پایان‌یافته پس از ۹۰ روز پاک‌سازی می‌شوند.
- نصب و آپدیت تا Healthy شدن واقعی ربات صبر می‌کنند؛ در شکست راه‌اندازی، وضعیت کانتینر و لاگ‌های bot/WARP چاپ می‌شود و پیام موفقیت کاذب نمایش داده نمی‌شود.

### پایداری هسته پایش از نسخه ۵.۳.۰
...
- تلاش مجدد کنترل‌شده در همان جریان ثبت، همراه قفل تک‌اجرا، Retry-After فارسی و بازیابی فقط-مثبت
...
- پیام همگانی پایدار مدیر با صف دسته‌ای، پیشرفت زنده، توقف، retry و ادامه خودکار پس از restart
...
flowchart LR
    ...
    D --> B
    A --> Q[("BroadcastCampaign و Delivery")]
    Q --> D
...
برای نصب مستقیم نسخه ۵.۴.۰ روی نصب موجود نیز همین فرمان کافی است:
...
نسخه فعال در پیام شروع و فرمان `/version` نمایش داده می‌شود. نسخه فعلی **5.4.0** است. پس از اولین اجرای موفق هر نسخه جدید، مدیر اصلی یک اعلان فارسی شامل شماره نسخه و فهرست تغییرات دریافت می‌کند. آخرین نسخه‌ای که اعلان شده در Redis نگهداری می‌شود تا با هر restart پیام تکراری ارسال نشود.
...
در نسخه ۵.۴.۰ دو نمای عملیاتی نیز برای هر کاربر وجود دارد:
...
- در همین حالت، دکمه `تلاش مجدد برای بررسی پیج` یک بررسی کنترل‌شده دیگر انجام می‌دهد. کلیک سریع‌تر از فاصله مجاز درخواست تازه نمی‌سازد و زمان انتظار دقیق را نشان می‌دهد؛ شکست تلاش مجدد نیز وضعیت انتخاب‌شده یا رکورد قبلی را تغییر نمی‌دهد.
...
- ساخت پیام همگانی متنی با حفظ قالب‌بندی تلگرام، پیش‌نمایش و تأیید نهایی؛ مشاهده پیشرفت، توقف با تأیید جداگانه و تلاش دوباره تحویل‌های ناموفق
...
- در مرحله ثبت، ۴۰۴ یک مسیر به‌تنهایی شاهد نبودن پیج نیست؛ اجرای مسیرهای باقی‌مانده ادامه می‌یابد، ۲۰۰ معتبر بر آن مقدم است و نتیجه منفی فقط پس از اجماع مستقل ارائه می‌شود.
...
Farstar Warner 5.4.0 is an asynchronous, OSINT-only Telegram bot using public Web Profile metadata, exact-match guest search, Embed HTML, and Playwright fallback. It never collects Instagram passwords, cookies, sessions, or Meta tokens. The Telegram interface is entirely in Persian; the Ubuntu installer is in English.
...
- A rate-controlled registration retry action with Persian Retry-After feedback and positive-only recovery during upstream incidents
- Independent-route 404 consensus; a valid 200 response always takes precedence over an earlier negative edge response
...
- Durable administrator broadcasts with preview, confirmation, progress, cancellation, retry, and restart recovery
