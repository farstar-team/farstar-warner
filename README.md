...
نسخه فعال در پیام شروع و فرمان `/version` نمایش داده می‌شود. پس از اولین اجرای موفق هر نسخه جدید، مدیر اصلی یک اعلان فارسی شامل شماره نسخه و فهرست تغییرات دریافت می‌کند. آخرین نسخه‌ای که اعلان شده در Redis نگهداری می‌شود تا با هر restart پیام تکراری ارسال نشود.
...
- اگر پیج فعال باشد، Chromium نمای عمومی را رندر می‌کند و ربات یک کارت تصویری اختصاصی برای تأیید کاربر می‌فرستد. اگر اینستاگرام جزئیات تصویر را به مرورگر سرور ندهد ولی پاسخ وضعیت قطعی و فعال باشد، کارت حداقلی همراه لینک مستقیم پیج نمایش داده می‌شود و کاربر همچنان می‌تواند پس از بررسی دستی آن را ثبت کند.
